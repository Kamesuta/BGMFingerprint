...
namespace SoundFingerprinting.Extensions.LMDB
{
    public sealed class LMDBModelService : IModelService, IDisposable
    {
        ...
        {
            var fingerprints = hashes.Audio.ToList();
            if (fingerprints.Count == 0)
            {
                return;
            }

            var trackData = TrackDao.InsertTrack(trackInfo, hashes.Audio.DurationInSeconds);
            SubFingerprintDao.InsertHashDataForTrack(hashes.Audio, trackData.TrackReference);
        }
        ...
        public void UpdateTrack(TrackInfo trackInfo)
        {
            ...
            var hashes = new AVHashes(new Hashes(subFingerprints.Select(subFingerprint => new HashedFingerprint(subFingerprint.Hashes, subFingerprint.SequenceNumber, subFingerprint.SequenceAt, subFingerprint.OriginalPoint)), track.Length, track.MediaType), null);
